...
class _MapScreenState extends State<MapScreen> {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['candidates'] != null && data['candidates'].isNotEmpty) {
          ...
          if(mounted) {
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('「$query」へ移動しました')));
          }
        } else {
           _zoomToFitResults();
        }
  ...
  Future<void> _determinePosition({bool silent = false}) async {
    ...
    try {
      ...
      setState(() {
        _userMarker = Marker(
          markerId: const MarkerId('my_location_pin'),
          position: LatLng(position.latitude, position.longitude),
          // 自分は「青（Blue）」
          icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueBlue),
          infoWindow: const InfoWindow(title: "あなたの現在地"),
          zIndex: 1000.0,
        );
      });
      ...
      if (!silent) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('現在地を表示しました'), backgroundColor: Colors.blue));
      }
    } catch (e) {
  ...
  void _showHotelDetails(Hotel hotel) {
    ...
    Widget sectionTitle(String title) {
      return Padding(padding: const EdgeInsets.only(top: 16, bottom: 8), child: Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.green)));
    }
      ...
      body: Stack(
        children: [
          GoogleMap(
            ...
            // ★色付きホテルマーカーと自分用マーカーを合体
            markers: _hotelMarkers.union(
              _userMarker != null ? {_userMarker!} : {}
            ),
