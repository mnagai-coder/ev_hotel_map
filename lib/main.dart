...
class _MapScreenState extends State<MapScreen> {
  ...
  Set<Marker> _hotelMarkers = {}; // ホテルのピン
  Marker? _userMarker;            // ★自分用のピン（これを復活！）
  ...
  String _statusMessage = "v4.0 手動ピン版";
    ...
    try {
      ...
      setState(() {
        _userMarker = Marker(
          ...
          icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueBlue), // 必ず青にする
          infoWindow: const InfoWindow(title: "現在地"),
          zIndex: 1000, // 他のピンより手前に表示
        );
  ...
  void _applyFilter() {
    setState(() {
      if (_selectedFilter == 'すべて') { _filteredHotels = _allHotels; } else { _filteredHotels = _allHotels.where((hotel) { final target = "${hotel.evType} ${hotel.output} ${hotel.category}"; return target.contains(_selectedFilter); }).toList(); }
      _createMarkers();
    });
  }
  ...
  void _showHotelDetails(Hotel hotel) {
    ...
    Widget sectionTitle(String title) => Padding(padding: const EdgeInsets.only(top: 16, bottom: 8), child: Text(title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.purple)));
      ...
      body: Stack(
        children: [
          GoogleMap(
            ...
            myLocationEnabled: true, // 念のため両方ON
          ...
          Positioned(bottom: 30, right: 20, child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Container(padding: const EdgeInsets.all(8), color: Colors.white70, child: Text(_statusMessage, style: const TextStyle(fontSize: 10))),
            const SizedBox(height: 8),
            FloatingActionButton(backgroundColor: Colors.purple, child: const Icon(Icons.my_location, color: Colors.white), onPressed: () { _determinePosition(); }),
          ])),
        ],
      ),
    );
